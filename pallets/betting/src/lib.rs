...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type AssetManager: Create<<Self as frame_system::Config>::AccountId>
			+ Mutate<<Self as frame_system::Config>::AccountId, Balance = u128, AssetId = u32>
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_round(
			...
			let who = T::Lookup::lookup(who)?;
				...
				duration > Zero::zero(),
				<Error<T>>::DurationMustGreaterThanZero
			);
		...
		pub fn claim(
			...
			let total_reward_u128: u128 = betting_round
				.total
				.saturating_sub(total_amount_bet)
				.saturated_into();
