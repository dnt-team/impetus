	...
	BalanceOf<Runtime>: TryFrom<U256> + Into<U256>
{
	...
	fn buy_ticket(handle: &mut impl PrecompileHandle, number: u8, amount: U256) -> EvmResult {
		let amount = Self::u256_to_amount(amount).in_field("amount")?;
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(handle.context().caller);
		let call = pallet_lucky_number::Call::<Runtime>::buy_ticket { number, amount };
	...
	fn claim_reward(handle: &mut impl PrecompileHandle, who: Address, round: u32, number: u8) -> EvmResult {
